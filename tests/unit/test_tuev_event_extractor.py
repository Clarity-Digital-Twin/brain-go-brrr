...
class TestTUEVEventExtractor:
    ...
    def test_channel_order_matches_reference(self):
        ...
        assert extractor.TUEV_CHANNELS_REF == expected_channels
    ...
    def test_filtering_and_resampling(self, mock_read_raw):
        ...
        mock_raw.filter.assert_called_once_with(
            l_freq=0.1, h_freq=75.0, verbose=False
        )
        mock_raw.notch_filter.assert_called_once_with(
            freqs=50.0, verbose=False
        )
        mock_raw.resample.assert_called_once_with(
            200, verbose=False  # Must resample to 200Hz
        )
