...
class TestEdfLoader:
    ...
    def test_load_with_kwargs(self, mock_mne):
        ...
        result = load_edf_safe("test.edf", preload=True, verbose=False, stim_channel=None)
        
        # Verify kwargs were passed
        call_args = mock_mne.io.read_raw_edf.call_args
        assert call_args[1].get('preload') == True
        assert call_args[1].get('verbose') == False
        assert call_args[1].get('stim_channel') == None