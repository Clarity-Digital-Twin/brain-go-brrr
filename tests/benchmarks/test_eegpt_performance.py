...
def eegpt_model_cpu():
    ...
    config = EEGPTConfig()
    model = EEGPTModel(checkpoint_path=None, config=config, device=torch.device("cpu"), auto_load=False)
    # Initialize architecture without loading checkpoint
...
def eegpt_model_gpu():
    ...
    config = EEGPTConfig()
    model = EEGPTModel(checkpoint_path=None, config=config, device=torch.device("cuda"), auto_load=False)
    model._initialize_model()
    return model
...
class TestPerformanceComparison:
    ...
    def test_cpu_vs_gpu_single_window(self, benchmark, eegpt_model_cpu, eegpt_model_gpu, single_eeg_window):
        ...
        # Benchmark CPU
        cpu_time = benchmark(lambda: eegpt_model_cpu.extract_features(single_eeg_window))
