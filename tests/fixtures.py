...
def old_channel_names() -> list[str]:
    ...
    return [
        'FP1', 'FP2', 'F7', 'F3', 'FZ', 'F4', 'F8',
        'T3', 'C3', 'CZ', 'C4', 'T4',  # T3/T4 instead of T7/T8
        'T5', 'P3', 'PZ', 'P4', 'T6',  # T5/T6 instead of P7/P8
        'O1', 'O2'
    ]
...
def mock_eeg_data(rng: np.random.Generator) -> np.ndarray:
    ...
    spectrum[positive_freqs] = rng.standard_normal(positive_freqs.sum()) + \
                              1j * rng.standard_normal(positive_freqs.sum())
