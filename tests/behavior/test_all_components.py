...
class TestApplicationBehavior:
    ...
    def test_yasa_sleep_staging(self, test_eeg_data):
        ...
        stages, confidences, metrics = stager.stage_sleep(
            eeg_data=eeg_array,
            sfreq=test_eeg_data.info['sfreq'],
            ch_names=test_eeg_data.ch_names
        )
    ...
    @pytest.mark.skipif(
        not os.getenv("BGB_ABN_MODEL") and 
        not Path("experiments/eegpt_linear_probe/output/tuab_4s_paper_target_BULLETPROOF_20250809_073159/best_model.pt").exists(),
