...
class TestAutoRejectEEGPTIntegration:
        ...
        for i in range(10):
            ...
            if i % 3 == 0:  # Add eye blinks
                ...
                for t in blink_times:
                    data[:4, int(t*sfreq):int((t+0.5)*sfreq)] += np.random.randn(4, int(0.5*sfreq)) * 200e-6
    ...
    def test_dataset_initialization_with_autoreject(self, mock_tuab_config, mock_edf_files, tmp_path):
