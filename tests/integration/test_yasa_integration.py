...
class TestYASAIntegration:
    ...
    def test_yasa_with_sleep_edf_data(self, sleep_edf_raw_cropped):
        ...
        if data.shape[1] / sfreq >= 30:
            stages, confidences, metrics = stager.stage_sleep(data, sfreq, ch_names)
    ...
    def test_pipeline_yasa_vs_mock(self, mock_eeg_data):
        ...
        # Test with YASA
        config_yasa = PipelineConfig(enable_sleep_staging=True, use_yasa_sleep_staging=True)
        pipeline_yasa = HierarchicalEEGAnalyzer(config_yasa)
        result_yasa = pipeline_yasa.analyze(data)

        # Test with mock
        config_mock = PipelineConfig(enable_sleep_staging=True, use_yasa_sleep_staging=False)
    ...
    def test_yasa_error_handling(self):
        ...
        config = PipelineConfig(enable_sleep_staging=True, use_yasa_sleep_staging=True)
