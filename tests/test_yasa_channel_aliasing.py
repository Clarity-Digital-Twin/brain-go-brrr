...
def test_channel_aliasing_applied(sleep_edf_data):
    ...
    stages, confidences, metrics = stager.stage_sleep(
        sleep_edf_data['data'],
        sleep_edf_data['sfreq'],
        sleep_edf_data['ch_names']
    )
...
def test_multiple_stages_detected(sleep_edf_data):
    ...
    # Should detect at least 2 different stages in 10 minutes
    unique_stages = set(stages)
    assert len(unique_stages) >= 2, f"Only detected stages: {unique_stages}"
...
def test_custom_channel_map():
    ...
    stages, confidences, metrics = stager.stage_sleep(
        data, 100, ch_names, channel_map=custom_map
    )
...
@pytest.mark.parametrize("n_channels,expected_success", [
