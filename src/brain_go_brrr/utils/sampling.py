...
def compute_class_weights(
    labels: np.ndarray,
    method: Literal["counts", "cb"] = "counts",
    beta: float = 0.9999
) -> torch.Tensor:
...
def print_class_distribution(labels: np.ndarray, class_names: Optional[dict] = None):
    ...
    print(f"Class imbalance ratio: {counts.max()}/{counts.min()} = {counts.max()/counts.min():.1f}:1")
...
def augment_minority_sample(
    ...
    if jitter_uv > 0:
        ...
        jitter_scale = 1.0 + (np.random.uniform(-jitter_uv, jitter_uv) / 1e6) / augmented.std().item()
