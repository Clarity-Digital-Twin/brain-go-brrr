...
from ..preprocessing.autoreject_adapter import SyntheticPositionGenerator, WindowEpochAdapter
...
OLD_TO_NEW_CHANNELS = {'T3': 'T7', 'T4': 'T8', 'T5': 'P7', 'T6': 'P8'}

# Reverse mapping
NEW_TO_OLD_CHANNELS = {'T7': 'T3', 'T8': 'T4', 'P7': 'T5', 'P8': 'T6'}
...
TUAB_20_CHANNELS = [
    ...
    'O1',
    'O2',
    'A1',  # Use A1 as 20th channel if available
]
...
class TUABEnhancedDataset(TUABDataset):
    ...
    def __init__(
        ...
        ar_cache_dir: str | Path | None = None,
        ...
        logger.info("Enhanced TUAB dataset initialized:")
        logger.info(f"  Window: {window_duration}s @ {sampling_rate}Hz")
        logger.info(
            f"  Stride: {window_stride}s ({window_stride/window_duration*100:.0f}% overlap)"
        )
        logger.info(f"  Filter: {bandpass_low}-{bandpass_high}Hz + {notch_freq}Hz notch")
        logger.info(
            f"  Channels: {len(channels)} ({self.use_old_naming and 'old' or 'new'} naming)"
        )
    ...
    def _initialize_autoreject(self) -> None:
        ...
        self.window_adapter = WindowEpochAdapter(
            window_duration=self.window_duration, window_stride=self.window_stride
        )
    ...
    def _load_edf_file(self, file_path: Path) -> npt.NDArray[np.float64]:
        ...
        if self.use_autoreject and self.position_generator is not None:
        ...
        # Get data (MNE returns float64 by default)
        data = raw.get_data()
    ...
    def _preprocess_raw(self, raw: mne.io.Raw) -> mne.io.Raw:
        ...
        if raw.info['sfreq'] != self.target_sampling_rate:
            raw.resample(
                sfreq=self.target_sampling_rate, npad='auto', n_jobs=self.n_jobs, verbose=False
            )
    ...
    def _extract_windows(
        self, raw: mne.io.Raw, label: int = 0
