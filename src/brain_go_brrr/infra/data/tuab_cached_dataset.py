...
class TUABCachedDataset(TUABDataset):
    ...
    def __init__(
        ...
        max_files: int | None = None,  # For testing!
    ) -> None:
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, int]:
        ...
        if current_samples != target_samples:
            ...
            if current_samples < target_samples:
                ...
                eeg_tensor = torch.nn.functional.pad(
                    eeg_tensor, (0, pad_size), mode='constant', value=0
                )
            else:
                # Trim to size
        ...
        if max_val > 1000:
            raise RuntimeError(
                f"Extreme values detected in sample {idx}: max={max_val:.2f}. Check normalization."
            )
