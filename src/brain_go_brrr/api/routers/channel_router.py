...
class ChannelRouter:
    ...
    def determine_analysis_method(
        cls,
        raw: mne.io.Raw,
        requested_method: Literal["auto", "yasa", "eegpt"] = "auto"
        ...
        if requested_method == "eegpt":
            ...
            if n_channels < cls.MIN_CHANNELS_YASA:
                raise ValueError(
                    f"YASA requires at least {cls.MIN_CHANNELS_YASA} channel, "
                    f"found {n_channels}"
                )
            ...
            if n_channels >= cls.MIN_CHANNELS_EEGPT:
                ...
                logger.info(
                    f"Auto-routing to EEGPT ({n_channels} channels available)"
                )
                return "eegpt", metadata
            else:
    ...
    @classmethod
    def validate_for_sleep_analysis(
        cls,
        raw: mne.io.Raw
    ) -> tuple[bool, str]:
