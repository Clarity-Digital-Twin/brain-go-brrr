...
class EEGSnippetMaker:
    ...
    def extract_fixed_snippets(
        ...
        while (
            current_time + snippet_length <= end_time and len(snippets) < self.max_snippets_per_file
        ):
    ...
    def create_snippet_report(
        ...
        report = {
            "summary": {
                ...
                "extraction_methods": list({s["extraction_method"] for s in snippets}),
                ...
                "average_quality_score": np.mean(
                    [s["classification"]["quality_score"] for s in processed_snippets]
                ),
            },
