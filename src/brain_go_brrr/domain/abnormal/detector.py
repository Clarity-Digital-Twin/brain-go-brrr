...
class AbnormalityDetector:
    ...
    def _predict_window(self, window: npt.NDArray[np.float64]) -> float:
        ...
        # Convert to tensor
        window_tensor = (
            torch.from_numpy(window).float().unsqueeze(0).to(self.device, non_blocking=True)
        )
        ...
        with torch.inference_mode():
            ...
            try:
                ...
                assert features.numel() in [512, 2048], (
                    f"Expected 512 or 2048 features, got {features.numel()}"
                )
