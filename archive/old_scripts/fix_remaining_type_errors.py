...
def main():
    ...
    if snippets_file.exists():
        ...
        content = content.replace(
            "return features_dict", "return features_dict  # type: ignore[no-any-return]"
        )
    ...
    if sleep_file.exists():
        ...
        content = content.replace("return y_pred", "return y_pred  # type: ignore[no-any-return]")
    ...
    if tuab_file.exists():
        ...
        content = content.replace(
            "return output_data  # type: ignore[no-any-return]", "return output_data"
        )
    ...
    if enhanced_file.exists():
        ...
        content = content.replace(
            "windows.append((window, self.current_label))", "windows.append((window, label))"
        )
    ...
    if task_file.exists():
        ...
        content = content.replace("return logits", "return logits  # type: ignore[no-any-return]")
        content = content.replace("return loss", "return loss  # type: ignore[no-any-return]")
    ...
    if robust_file.exists():
        ...
        # Fix .item() calls
        content = re.sub(
            r"self\.nan_count\.item\(\)", "self.nan_count.item()  # type: ignore[operator]", content
        )
        content = re.sub(
        ...
        # Fix .fill_ calls
        content = re.sub(
            r"self\.nan_count\.fill_\(", "self.nan_count.fill_(  # type: ignore[operator]", content
        )
        content = re.sub(
    ...
    if app_file.exists():
        ...
        content = content.replace(
            "route.endpoint,", "route.endpoint,  # type: ignore[attr-defined]"
        )
        content = content.replace(
            "methods=route.methods,", "methods=route.methods,  # type: ignore[attr-defined]"
        )
