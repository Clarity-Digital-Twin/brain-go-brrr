    ...
    for manifest_file in cache_dir.glob("manifest_*.json"):
        ...
        try:
            ...
            if prep["channels"] != expected_channels:
                errors.append(f"{split}: Wrong channels {prep['channels']} (need {expected_channels} for {corpus})")
            ...
            if manifest["data_stats"]["feature_dim"] != 2048:
                warnings.append(f"{split}: Feature dim {manifest['data_stats']['feature_dim']} != 2048")
            ...
            print(f"✓ {split} manifest: {manifest['data_stats']['n_windows']} windows, hash {manifest['config_hash'][:8]}")
