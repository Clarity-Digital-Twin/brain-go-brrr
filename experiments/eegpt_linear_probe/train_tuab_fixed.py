...
def train_epoch(model, probe, train_loader, optimizer, scheduler, device, config, epoch=0, output_dir=None, start_batch=0, global_step=0):
    """Train for one epoch with resume support."""
    ...
    for batch_idx, (data, labels) in enumerate(pbar):
        ...
        try:
            ...
            if batch_idx % 500 == 0 and batch_idx > 0:
                ...
                logger.info(f"Saved checkpoint at batch {batch_idx}, global_step {global_step}")
    ...
    # CRITICAL: Check for early epoch termination
    expected_batches = len(train_loader)
    actual = batch_idx + 1 if 'batch_idx' in locals() else 0
    if actual != expected_batches and start_batch == 0:
        logger.error(f"EARLY EPOCH TERMINATION: got {actual}/{expected_batches} batches!")
        logger.error("Check TUABMemoryMappedDataset.__len__ and collate_eeg_batch_fixed")
        raise RuntimeError(f"Early epoch termination detected: {actual}/{expected_batches}")
...
def main():
    ...
    logger.info(f"Train batches (len(train_loader)): {len(train_loader)}")
    logger.info(f"Batch size: {config['data']['batch_size']}")
    logger.info(f"Expected batches: {len(train_loader.dataset) // config['data']['batch_size']}")
    ...
    if args.resume:
        ...
        start_batch = checkpoint.get("batch_idx", -1) + 1  # Resume from next batch
        global_step = checkpoint.get("global_step", 0)
        best_val_auroc = checkpoint.get("best_val_auroc", 0)
        
        # If we completed the epoch, move to next epoch
        if start_batch >= len(train_loader):
            start_epoch += 1
            start_batch = 0
        
        logger.info(f"Resumed from epoch {start_epoch}, batch {start_batch}, global_step {global_step}")
    else:
        best_val_auroc = 0
    ...
    for epoch in range(start_epoch, config["training"]["max_epochs"]):
        ...
        # Train with proper resume support
        ...
        train_metrics, global_step = train_epoch(
            ...
            epoch, output_dir, start_batch=current_start_batch, global_step=global_step
        )
        logger.info(
        ...
        if (epoch + 1) % 2 == 0:  # Validate every 2 epochs
            ...
            if val_metrics["auroc"] > best_val_auroc:
                ...
                checkpoint = {
                    "epoch": epoch,
                    "batch_idx": len(train_loader) - 1,  # End of epoch
                    "global_step": global_step,
                    "probe_state_dict": probe.state_dict(),
                    "optimizer_state_dict": optimizer.state_dict(),
                    "scheduler_state_dict": scheduler.state_dict(),
