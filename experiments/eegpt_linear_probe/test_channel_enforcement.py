...
def test_channel_specs():
    ...
    assert len(tuab_proc.STANDARD_CHANNELS) == 19, f"TUAB should have 19 channels, got {len(tuab_proc.STANDARD_CHANNELS)}"
    ...
    assert len(tuev_proc.STANDARD_CHANNELS) == 20, f"TUEV should have 20 channels, got {len(tuev_proc.STANDARD_CHANNELS)}"
    ...
    assert 'Fpz' not in [ch.upper() for ch in tuev_proc.STANDARD_CHANNELS], "TUEV should NOT include Fpz"
    ...
    assert len(unique_channels) == 20, f"TUEV config should have 20 unique channels, got {len(unique_channels)}"
