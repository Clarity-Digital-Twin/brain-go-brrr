    ...
    for batch_idx, (data, labels) in enumerate(pbar):
        ...
        if should_step:
            ...
            grad_norm = torch.nn.utils.clip_grad_norm_(
                probe.parameters(), float(config["training"]["gradient_clip_val"])
            )
            ...
            if global_step == 1:
                ...
                if abs(current_lr - expected) > expected * 0.1:
                    logger.warning(
                        f"Initial LR mismatch: {current_lr:.6f} vs expected {expected:.6f}"
                    )
            ...
            if global_step == 100:
                ...
                if current_lr <= initial_lr:
                    logger.warning(
                        f"⚠️ Warmup not working! LR should be > {initial_lr:.6f}, got {current_lr:.6f}"
                    )
        ...
        if global_step > 0 and global_step % config["logging"]["log_every_n_steps"] == 0:
            logger.info(
                f"Step {global_step} - Loss: {(loss * accum_steps).item():.4f} - LR: {current_lr:.2e} - Grad norm: {grad_norm:.2f}"
            )
...
def main():
    ...
    # Log configuration
    logger.info(f"\n{'=' * 60}")
    logger.info("OneCycleLR Scheduler Configuration (BULLETPROOF):")
    logger.info(
    ...
    if global_step > 0:
        ...
        if global_step < warmup_end:
            logger.info("  ⬆️ Currently in WARMUP phase")
        else:
            logger.info("  ⬇️ Currently in ANNEALING phase")
    logger.info(f"{'=' * 60}\n")
    ...
    expected_final = (
        config["training"]["scheduler"]["max_lr"]
        / config["training"]["scheduler"]["final_div_factor"]
    )
