...
class LinearProbe(nn.Module):
    ...
    def __init__(
        self, input_dim=512, hidden_dim=128, n_classes=2, dropout=0.1, probe_type="linear"
    ):
        super().__init__()
...
def train_epoch(model, probe, train_loader, optimizer, scheduler, device, config):
    ...
    for batch_idx, (data, labels) in enumerate(pbar):
        ...
        if batch_idx % config["logging"]["log_every_n_steps"] == 0:
            logger.info(
                f"Batch {batch_idx}/{len(train_loader)} - Loss: {loss.item():.4f} - LR: {current_lr:.2e}"
            )
...
def main():
    ...
    parser.add_argument(
        "--resume",
        type=str,
        default="output/tuab_4s_paper_target_20250806_132743/best_model.pt",
    ...
    scheduler = torch.optim.lr_scheduler.OneCycleLR(
        ...
        else -1,  # Resume from correct position
    )

    logger.info(
        f"OneCycleLR initialized: {total_steps} total steps ({steps_per_epoch} steps/epoch * {config['training']['max_epochs']} epochs)"
    )
    if steps_completed > 0:
        logger.info(
            f"Resuming from step {steps_completed} / {total_steps} ({steps_completed / total_steps * 100:.1f}% complete)"
        )
    logger.info(
        f"LR schedule: {config['training']['scheduler']['max_lr'] / config['training']['scheduler']['div_factor']:.6f} → {config['training']['scheduler']['max_lr']:.6f} → {config['training']['scheduler']['max_lr'] / config['training']['scheduler']['final_div_factor']:.6f}"
    )
    ...
    for epoch in range(start_epoch, config["training"]["max_epochs"]):
        ...
        train_metrics = train_epoch(
            model, probe, train_loader, optimizer, scheduler, device, config
        )
        logger.info(
