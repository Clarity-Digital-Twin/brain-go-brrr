...
def build_4s_cache():
        ...
        try:
            ...
            if window_list:
                ...
                labels_tensor = torch.full(
                    (len(window_list),), file_info["label"], dtype=torch.int8
                )
    ...
    # Summary by split
    train_windows = sum(
        info["n_windows"] for info in cache_index["files"].values() if info["split"] == "train"
    )
    eval_windows = sum(
        info["n_windows"] for info in cache_index["files"].values() if info["split"] == "eval"
    )
