#!/usr/bin/env python
"""GPU training using only eval data split into train/val."""
...
def main():
    ...
    cfg.training.batch_size = 128  # GPU optimized
    ...
    cfg.training.num_workers = 8  # GPU optimized
    ...
    log_dir = Path(f"logs/eval_gpu_run_{timestamp}")

    logger.info("=" * 60)
    logger.info("GPU TRAINING (EVAL DATA SPLIT)")
    logger.info("=" * 60)
    ...
    # Load eval dataset and split it
    logger.info("Loading eval dataset and splitting into train/val...")
    full_dataset = TUABDataset(
        root_dir=dataset_root,
        split="eval",  # Use eval data
    ...
    logger.info(f"Total eval windows: {len(full_dataset)}")
    ...
    logger.info(f"Class weights: Normal={weight_normal:.3f}, Abnormal={weight_abnormal:.3f}")
