...
from tuev_dataset import TUEVCachedDataset, TUEVDataset
...
class TUEVLinearProbe(nn.Module):
    ...
    def __init__(self, eegpt_checkpoint: str, device: str = 'cuda'):
        ...
        self.eegpt = EEGPTWrapper(checkpoint_path=eegpt_checkpoint, device=device)
        ...
        # Linear probe (15×4×512 → 6)
        # 15 patches × 4 summary tokens × 512 dim = 30,720
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x: Input tensor of shape (batch, 23, 1000)
            
        Returns:
            Logits of shape (batch, 6)
        """
        # Verify input shape
        assert x.shape[1:] == (23, 1000), f"Wrong input shape: {x.shape}, expected (batch, 23, 1000)"
        
        # Channel reduction: 23 → 20
        x = self.channel_reducer(x)  # (batch, 20, 1000)
        x = F.gelu(self.bn1(x))
        
        # Temporal convolution (depthwise)
        x = self.temporal_conv(x)  # (batch, 20, 1000)
        x = F.gelu(self.bn2(x))
        
        # Dropout
        x = self.dropout(x)  # (batch, 20, 1000)
        
        # EEGPT encoder (frozen)
        with torch.no_grad():
            # EEGPT expects (batch, channels, time)
            features = self.eegpt.extract_features(x)  # (batch, 15, 4, 512)
        
        # Flatten and classify
        features = features.view(features.size(0), -1)  # (batch, 30720)
...
def main(args):
    ...
    if args.use_cache:
        ...
        train_dataset = TUEVCachedDataset(
            cache_dir=Path(config.data.cache_dir),
            split='train'
        )
        val_dataset = TUEVCachedDataset(
            cache_dir=Path(config.data.cache_dir),
            split='eval'
        )
    else:
