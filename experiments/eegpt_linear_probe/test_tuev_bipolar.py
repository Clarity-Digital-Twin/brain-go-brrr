...
import numpy as np
from datasets.tuev_dataset import compute_bipolar_derivation, TCP_BIPOLAR_PAIRS
...
def test_bipolar_derivation():
    ...
    n_samples = 1000
    ch_names = ['FP1-REF', 'F7-REF', 'T3-REF', 'T5-REF', 'O1-REF',
                 'FP2-REF', 'F8-REF', 'T4-REF', 'T6-REF', 'O2-REF',
                 'F3-REF', 'C3-REF', 'P3-REF', 'F4-REF', 'C4-REF', 
                 'P4-REF', 'FZ-REF', 'CZ-REF', 'PZ-REF', 'A1-REF', 'A2-REF']
    ...
    np.testing.assert_allclose(actual_fp1_f7, expected_fp1_f7, rtol=1e-5,
                               err_msg="FP1-F7 derivation incorrect")
    
    print(f"✓ Bipolar derivation test passed: {non_zero_channels}/23 channels active")
    print(f"✓ FP1-F7 derivation correctly computed as FP1-REF minus F7-REF")
