...
def create_manifest(
    ...
    manifest = {
        ...
        "corpus": {
            ...
            "split": split,
            "data_root": str(data_root.relative_to(Path.cwd()))
        ...
        "data_stats": {
            ...
            "expected_shape": [
                CHANNELS_TUAB if corpus == "TUAB" else CHANNELS_TUEV,
                WINDOW_SAMPLES,
            ],
...
def build_cache(
    corpus: str, split: str, data_root: Path, cache_dir: Path, force_rebuild: bool = False
) -> None:
    ...
    # Create dataset - use correct class for each corpus
    if corpus == "TUAB":
        dataset = TUABDataset(
            root_dir=data_root,
            split=split,
    ...
    logger.info("✓ Cache ready for use!")
...
def main():
    ...
    parser.add_argument(
        '--data-root', type=str, required=True, help='Root directory containing EDF files'
    )
    ...
    parser.add_argument(
        '--force-rebuild', action='store_true', help='Force rebuilding even if cache exists'
    )
