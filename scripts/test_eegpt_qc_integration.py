...
from brain_go_brrr  # noqa: E402.core.quality import (  # noqa: E402 - Import after sys.path modification for script execution
...
def test_eegpt_qc_integration():
    ...
    for st in spike_times:
        data_abnormal[:, st : st + 10] += (
            np.random.randn(n_channels, min(10, len(times) - st)) * 200e-6
        )
    ...
    try:
        ...
        print(
            f"   Abnormality score: {result_abnormal['quality_metrics']['abnormality_score']:.3f}"
        )
    ...
    if (
        result_abnormal["quality_metrics"]["abnormality_score"]
        > result_normal["quality_metrics"]["abnormality_score"]
    ):
    ...
    if "processing_info" in result_normal:
        ...
        print(
            f"   Autoreject available: {result_normal['processing_info']['autoreject_available']}"
        )
    ...
    if edf_path.exists():
        try:
            ...
            print(
                f"   Abnormality score: {result_real['quality_metrics']['abnormality_score']:.3f}"
            )
