...
def main():
    ...
    if cfg.data.cache_mode == "readonly" and cache_misses > 0:
        logger.error("❌ CACHE MISSES in readonly mode! Check configuration")
        sys.exit(1)
    elif avg_batch_time > 2.0:
        logger.warning("⚠️  Slow loading detected - cache may not be working")
        sys.exit(1)
    else:
        logger.info("✅ Cache working properly!")
        sys.exit(0)
