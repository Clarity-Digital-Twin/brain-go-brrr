...
VALID_MODULES = {
    ...
    'core': ['config', 'logger', 'exceptions', 'abnormal', 'sleep', 'quality', 'edf', 'pipeline', 'jobs'],
...
def analyze_test_file(file_path: Path) -> Dict[str, any]:
    ...
    if tested_modules:
        is_valid = True
    elif any(module in filename.lower() for module_list in VALID_MODULES.values() for module in module_list):
        is_valid = True
    elif any(keyword in filename.lower() for keyword in VALID_KEYWORDS):
        is_valid = True
    ...
    for file in test_root.rglob('*.py'):
        if file.name.startswith(('test_', '_test')) or file.name.endswith('_test.py'):
            ...
            if 'conftest' not in file.name:
                ...
                if depth > 0:
                    rel_import = '../' * depth
                    content = re.sub(r'from conftest import', f'from {rel_import}conftest import', content)
