            ...
            if i + 1 < len(lines):
                ...
                if 'async def' in next_line or 'def' in next_line:
                    ...
                    if func_match and '-> ' not in next_line:
                        ...
                        print(f"Warning: {file_path}:{i+2} - Function {func_match.group(1)} missing return type")
...
def fix_unused_type_ignores(file_path: Path) -> None:
    ...
    # Remove unused type: ignore comments
    new_content = re.sub(r'\s*#\s*type:\s*ignore(?:\[[^\]]+\])?\s*$', '', content, flags=re.MULTILINE)
...
def main():
    ...
    # Read type errors
    with open('/tmp/type_errors.txt', 'r') as f:
        errors = f.readlines()
