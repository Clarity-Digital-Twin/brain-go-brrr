...
def main():
    ...
    print(
        f"  WITHOUT Oz: {len(results) - with_oz} files ({(len(results) - with_oz) / len(results) * 100:.1f}%)"
    )
    ...
    print(
        f"  WITHOUT Fz: {len(results) - with_fz} files ({(len(results) - with_fz) / len(results) * 100:.1f}%)"
    )
