...
class EEGPipeline:
    ...
    def load_eeg_data(self, file_path: Path, loader: str = "mne") -> mne.io.Raw:
        ...
        logger.info(
            f"Loaded EEG data: {raw.info['nchan']} channels, {raw.info['sfreq']} Hz, {raw.times[-1]:.1f}s"
        )
        return raw

    def run_quality_control(self, raw: mne.io.Raw) -> dict:
    ...
    def run_sleep_analysis(self, raw: mne.io.Raw) -> dict:
    ...
    def run_snippet_extraction(self, raw: mne.io.Raw, anomaly_scores: np.ndarray = None) -> dict:
        ...
        if anomaly_scores is not None:
            anomaly_snippets = self.snippet_maker.extract_anomaly_snippets(raw, anomaly_scores)
        ...
        snippet_report = self.snippet_maker.create_snippet_report(
            all_snippets, include_features=True, include_eegpt=True
        )
    ...
    def generate_comprehensive_report(
        self, raw: mne.io.Raw, qc_report: dict, sleep_report: dict, snippet_report: dict
    ) -> dict:
        ...
        report = {
            ...
            "summary": {
                ...
                "sleep_quality": sleep_report.get("quality_metrics", {}).get(
                    "quality_grade", "N/A"
                ),
...
def main():
    ...
    if sample_file.exists():
        ...
        info = mne.create_info(
            ch_names=["Fp1", "Fp2", "C3", "C4", "O1", "O2"], sfreq=250, ch_types="eeg"
        )
