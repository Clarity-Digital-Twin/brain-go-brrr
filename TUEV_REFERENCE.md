...
## ⚠️ Stability Notes (Single-GPU/No-DS)
- NaNs observed under small-batch, single-GPU runs without DeepSpeed. Symptoms: rising/NaN `grad_norm` then `Loss is nan` around mid-epochs.
- Recommended parity/stability:
  - Use DeepSpeed (`--enable_deepspeed`) with effective batch ≈800 (2×400) as in the launch script, or simulate with `--update_freq` to match effective batch if using fewer GPUs.
  - Keep warmup (`--warmup_epochs 5`) and cosine LR to `--min_lr 1e-6`.
  - Optional diagnostic when not using DeepSpeed: enable `--clip_grad 1.0` to bound gradients.
  - Always add `--save_ckpt` so best-on-val is saved even if a later epoch destabilizes.
